...
class TestPublish_Tweet extends WP_UnitTestCase {
	...
	public function test_get_upload_data_media_id() {
		...
		add_filter( 'autotweet_attached_image', '__return_false' );
		$this->assertNull( $this->publish_tweet->get_upload_data_media_id( $post ) );
		remove_filter( 'autotweet_attached_image', '__return_false' );

		$filter_media_upload_id = function() {
			return 999;
		};

		add_filter( 'autotweet_pre_media_upload', $filter_media_upload_id );
		$this->assertEquals( 999, $this->publish_tweet->get_upload_data_media_id( $post ) );
		remove_filter( 'autotweet_pre_media_upload', $filter_media_upload_id );
	}
	...
	public function test_get_largest_acceptable_imagel() {
		...
		add_filter( 'autotweet_max_image_size', $set_150kb_max_filesize );
		...
		remove_filter( 'autotweet_max_image_size', $set_150kb_max_filesize );

		$set_1kb_max_filesize = function() {
			return 1000;
		};
		add_filter( 'autotweet_max_image_size', $set_1kb_max_filesize );
		$file = $this->publish_tweet->get_largest_acceptable_image(
			get_attached_file( $attachment ),
			wp_get_attachment_metadata( $attachment )['sizes']
		);
		...
		remove_filter( 'autotweet_max_image_size', $set_1kb_max_filesize );
	}
}
