...
class TestPostMeta extends WP_UnitTestCase {
	...
	public function test_get_tweet_status_message() {
		...
		add_filter( 'autoshare_meta', $published_filter, 10, 3 );
		$this->assertEquals(
			[
		...
		remove_filter( 'autoshare_meta', $published_filter );
		...
		add_filter( 'autoshare_meta', $failed_filter, 10, 3 );
		$this->assertEquals(
			[
		...
		remove_filter( 'autoshare_meta', $failed_filter );
		...
		add_filter( 'autoshare_meta', $unknown_filter, 10, 3 );
		$this->assertEquals(
			[
				'message' => 'There was an error.',
			],
		...
		remove_filter( 'autoshare_meta', $unknown_filter );
		...
		add_filter( 'autoshare_meta', $other_filter, 10, 3 );
		$this->assertEquals(
			[
		...
		remove_filter( 'autoshare_meta', $other_filter );
	}
